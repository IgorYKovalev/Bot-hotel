...
@bot.callback_query_handler(func=lambda call: call.data.startswith('{'), state=TravelInfoState.show_hotels)
    ...
    if hotels_list:
        ...
        if hotels_list_length == 0:
            bot.send_message(chat_id, 'К сожалению, по твоему запросу не найдено ни одного отеля.\n'
                                      'Хочешь повторить поиск?', reply_markup=default_keyboard())
        ...
        bot.send_message(chat_id,
                         'Кажется что-то пошло не так! Попробуй повторить поиск.\n'
                         'Если ошибка будет повторяться, свяжись с - @IgorKovalev',
                         reply_markup=default_keyboard())
...
def show_hotel_photo(callback: CallbackQuery) -> None:
    ...
    if photos_dict:
        ...
        bot.send_message(callback.message.chat.id,
                         'Кажется что-то пошло не так! Попробуй повторить поиск.\n'
                         'Если ошибка будет повторяться снова, свяжись с - @IgorKovalev',
                         reply_markup=default_keyboard())
    bot.answer_callback_query(callback.id)


@bot.callback_query_handler(func=lambda call: call.data == 'HIDE', state=TravelInfoState.show_hotels)
